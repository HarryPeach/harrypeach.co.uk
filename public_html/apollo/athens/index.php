<?php
/*
Athens - A simple PHP web proxy. <https://github.com/joshdick/Athens>
Written and maintained by Joshua Dick <http://joshdick.net>.
Athens is licensed under the GNU GPL v3 <http://www.gnu.org/licenses/gpl.html>.
*/
/****************************** START CONFIGURATION ******************************/
...
$forceCORS = false;
//URL that will be used as an example in the instructional text on the Athens landing page,
//and that will be proxied when pressing the 'Proxy It!' button on the landing page
//if the URL form is left blank.
$exampleURL = 'https://harrypeach.co.uk';
/****************************** END CONFIGURATION ******************************/
ob_start("ob_gzhandler");
if (version_compare(PHP_VERSION, "5.4.7", "<")) {
    die("Athens requires PHP version 5.4.7 or later.");
}
if (!function_exists("curl_init")) die("Athens requires PHP's cURL extension. Please install/enable it on your server and try again.");
...
function makeRequest($url) {
  ...
  if (empty($user_agent)) {
    $user_agent = "Mozilla/5.0 (compatible; Athens)";
  }
  $ch = curl_init();
  ...
  switch ($_SERVER["REQUEST_METHOD"]) {
    case "POST":
      ...
      //If the AthensFormAction field appears in the POST data, remove it so the destination server doesn't receive it.
      $postData = Array();
      parse_str(file_get_contents("php://input"), $postData);
      if (isset($postData["AthensFormAction"])) {
        unset($postData["AthensFormAction"]);
      }
...
if (isset($_POST["AthensFormAction"])) {
  $url = $_POST["AthensFormAction"];
  unset($_POST["AthensFormAction"]);
} else {
  $queryParams = Array();
  parse_str($_SERVER["QUERY_STRING"], $queryParams);
  //If the AthensFormAction field appears in the query string, make $url start with its value, and rebuild the the query string without it.
  if (isset($queryParams["AthensFormAction"])) {
    $formAction = $queryParams["AthensFormAction"];
    unset($queryParams["AthensFormAction"]);
...
if (empty($url)) {
    die("<html><head><link rel=\"stylesheet\" type=\"text/css\" href=\"../assets/css/core.css\"><title>Athens</title></head><body><div id=\"centre\"><h1>Welcome to <span class=\"anaglyph\">Athens</span></h1><br /><form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $exampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"proceed\" /></form></div></body></html>");
    ...
    //See https://github.com/joshdick/Athens/pull/14
    $pos = strpos($url, ":/");
...
if (empty($scheme)) {
  ...
  if (strpos($url, "//") === 0) {
    ...
    die('Error: Detected a "' . $scheme . '" URL. Athens exclusively supports http[s] URLs.');
}
...
if (stripos($contentType, "text/html") !== false) {
  ...
  foreach($xpath->query("//form") as $form) {
    ...
    $actionInput->appendXML('<input type="hidden" name="AthensFormAction" value="' . htmlspecialchars($action) . '" />');
  ...
  echo "<!-- Proxified page constructed by Athens -->\n" . $doc->saveHTML();
